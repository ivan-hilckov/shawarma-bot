...
Если возникают проблемы с запуском на продакшен сервере (ошибка `Cannot find module '/app/dist/api/server.js'`):
...
# Проверка локальной сборки TypeScript
npm run build
ls -la dist/api/server.js

# Тестирование Docker сборки
docker build --no-cache -t shawarma-bot-debug .
docker run --rm shawarma-bot-debug ls -la /app/dist/api/
```

#### Диагностика на сервере

Обновленный скрипт деплоя `deploy.sh` включает автоматическую диагностику:

- ✅ Проверка исходных файлов (`src/api/server.ts`, `tsconfig.json`)
- ✅ Тестирование сборки TypeScript на сервере
- ✅ Детальный вывод процесса сборки Docker
- ✅ Автоматическое переустановка зависимостей при ошибках

#### Быстрое исправление проблем

Если процесс завис на очистке Docker кэша или нужно быстро исправить проблему:

```bash
# Экстренное быстрое исправление (30 секунд)
./quick-fix.sh

# Полное исправление с диагностикой (2-3 минуты)
./fix-build-issue.sh
...
- 🚀 `quick-fix.sh` - если нужно быстро перезапустить только API контейнер
- 🔧 `fix-build-issue.sh` - стандартное исправление с полной переустановкой
- 🔍 `debug-build.sh` - детальная диагностика для сложных случаев

#### Альтернативная сборка

Если основной Dockerfile не работает, используйте альтернативный:

```bash
# Сборка с альтернативным Dockerfile
docker build -f Dockerfile.alternative -t shawarma-bot-alt .

# Обновление docker-compose.yml для использования альтернативного образа
# Замените 'build: .' на 'image: shawarma-bot-alt'
```

#### Возможные причины проблем

1. **Отсутствие TypeScript зависимостей** - проверьте `package.json`
2. **Проблемы с правами доступа** - проверьте владельца файлов
3. **Неполное копирование файлов** - проверьте `.dockerignore`
4. **Ошибки компиляции TypeScript** - проверьте `tsconfig.json`
5. **Проблемы с npm cache** - очистите кэш: `npm cache clean --force`
...
docker exec -it shawarma-api ls -la /app/dist/

# Проверка процесса сборки
docker exec -it shawarma-api npm run build
